...
public class BaseCommand implements CommandExecutor {
	...
	protected BaseCommand(){
		...
		playerHelper = new PlayerHelper(plugin);
	}
	...
	public final boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings){
		...
		}catch(Exception e){ //TODO Custom exception for commands
			sendResponse(commandSender, "Error: " + e.getMessage(), ChatColor.DARK_RED);
			return false;
		}
	}
	
	public boolean onCommandAll(CommandSender commandSender, Command command, String s, String[] strings) throws MCAdminCommandException {
	...
	public boolean onCommandConsole(CommandSender commandSender, Command command, String s, String[] strings) throws MCAdminCommandException {
		sendResponse(commandSender, "Sorry, this command cannot be used from the console!", ChatColor.DARK_RED);
		return true;
	}
	
	public boolean onCommandPlayer(Player player, Command command, String s, String[] strings) throws MCAdminCommandException {
		sendResponse(player, "Sorry, this command cannot be used by a player!", ChatColor.DARK_RED);
		return true;
	}
	...
	public static void registerCommands(){
		...
		//registerCommand(new CommandThatExtendsBaseCommand()); ??? does this work for commands outside of the package...
	}
	...
	private static void registerCommand(Class<? extends BaseCommand> commandClass){
		try {
			...
			if(!commandClass.isAnnotationPresent(Name.class))
				return;
			MCAdmin.instance.getCommand(commandClass.getAnnotation(Name.class).value()).setExecutor(command);
		}catch(Exception e){}
	}
	...
	protected void sendResponse(CommandSender cmd, String msg, ChatColor color){
		cmd.sendMessage(color + "[MCAdmin] " + ChatColor.WHITE + msg);
	}
}
