...
public abstract class ICommand {
	...
	protected MCAdmin plugin;
	protected PlayerHelper playerHelper;

	protected ICommand() {
		this(MCAdmin.instance.commandSystem);
	}
	...
	protected String parseFlags(String argStr) throws MCAdminCommandException {
	...
	protected String[] parseFlags(String[] args) throws MCAdminCommandException {
		int nextArg = 0;
		...
		while (nextArg < args.length) {
			...
			for (int i = 1; i < arg.length(); ++i) {
				...
				if (flagType == null)
					throw new MCAdminCommandException("Invalid flag '" + flagName + "' specified.");
				...
				case STRING:
					...
					if (nextArg >= args.length)
						throw new MCAdminCommandException("No value specified for " + flagName + " flag.");
	...
	public void Run(Player player, String[] args, String argStr, String commandName) throws MCAdminCommandException {
	}

	public void run(CommandSender commandSender, String[] args, String argStr, String commandName)
			throws MCAdminCommandException {
		Run(asPlayer(commandSender), args, argStr, commandName);
	}

	public static Player asPlayer(CommandSender commandSender) throws MCAdminCommandException {
		if (!(commandSender instanceof Player))
			throw new MCAdminCommandException("This command can only be run as a player.");
	...
	public static Location getCommandSenderLocation(CommandSender commandSender, boolean elevated)
		...
		if (location == null)
			throw new MCAdminCommandException("This command can only be run as a player or a command block.");
	...
	public static CraftPlayer asCraftPlayer(CommandSender commandSender) throws MCAdminCommandException {
		if (!(commandSender instanceof Player))
			throw new MCAdminCommandException("This command can only be run as a player.");

		if (!(commandSender instanceof CraftPlayer))
			throw new MCAdminCommandException("This command can only be run on CraftBukkit.");

		return (CraftPlayer) commandSender;
	}

	public static EntityPlayer asNotchPlayer(CommandSender commandSender) throws MCAdminCommandException {
